...
resource "null_resource" "trigger_migration" {
  ...
  provisioner "local-exec" {
    command = "aws lambda invoke --function-name ${module.migration_lambda.lambda_function_name} --invocation-type RequestResponse --region ${data.aws_region.current.id} response.json && cat response.json"
  }
}
